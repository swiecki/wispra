...
pub const TranscriptionLog = struct {
    ...
    pub fn getStats(self: *const TranscriptionLog) !struct {
        ...
        while (lines_iter.next()) |line| {
            ...
            if (TranscriptionEntry.fromLogLine(self.allocator, trimmed)) |entry| {
                ...
                const text_to_count = if (entry.corrected_text) |corrected| corrected else entry.raw_text;
                var word_iter = std.mem.tokenize(u8, text_to_count, " \t\n\r");
                while (word_iter.next()) |_| {
                    total_words += 1;
                }
                
                if (earliest_timestamp == null or entry.timestamp < earliest_timestamp.?) {
                    earliest_timestamp = entry.timestamp;
                }
                if (latest_timestamp == null or entry.timestamp > latest_timestamp.?) {
